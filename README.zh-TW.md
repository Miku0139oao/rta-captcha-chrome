...
3. 按「安裝」。
4. 從 `https://partner.rta-os.com/` 進入登入（會跳轉 SSO / iframe 也可以）。右下角應出現「RTA OCR 已載入」。驗證碼圖片出現後會自動填入；登入仍由你送出。
5. 若已安裝舊版腳本，到 Tampermonkey 儀表板刪除後，再打開上面的 Raw 連結重裝（版本需為 1.0.2）。
